  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      <svg viewBox="0 0 440 300" style={{ width: '100%', height: '100%', overflow: 'visible' }}>
        <defs>
          ...
          <marker id="la-arrow-active" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="8" markerHeight="8" orient="auto-start-reverse">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="#00F5C4" />
          </marker>
        ...
        {EDGES.map((e, i) => {
          ...
          return (
            <g key={i}>
              ...
              />
              {isActive && (
                <circle r="4" fill="#7CFFB2" filter="url(#la-glow)">
              ...
              <g transform={`translate(${cx} ${cy})`}>
                <rect x="-11" y="-10" width="22" height="18" rx="5"
                      fill={isActive ? 'rgba(0,245,196,0.16)' : 'rgba(18,25,35,0.92)'}
                      stroke={isActive ? '#00F5C4' : '#1F3B73'} strokeWidth="1" />
                <text textAnchor="middle" y="4" fontFamily="JetBrains Mono, monospace" fontSize="11"
                      fill={isActive ? '#E6F1FF' : '#7A8CA3'} fontWeight="600">{e.label}</text>
              </g>
            </g>
          );
        })}
        ...
        <g>
          <path d={`M ${POS.q0.x - 52} ${POS.q0.y} L ${POS.q0.x - 22} ${POS.q0.y}`} stroke="#7A8CA3" strokeWidth="1.5" markerEnd="url(#la-arrow)" fill="none"/>
          <text x={POS.q0.x - 58} y={POS.q0.y - 8} textAnchor="end" fontSize="10" fill="#7A8CA3" fontFamily="JetBrains Mono, monospace">start</text>
        </g>
        ...
        {DFA.states.map((s) => {
          ...
          return (
            <g key={s} transform={`translate(${x} ${y})`}>
              {isCurrent && (
                <>
                  <circle r="32" fill="none" stroke="#00F5C4" strokeWidth="1" opacity="0.6">
                    <animate attributeName="r" values="22;44;22" dur="1.6s" repeatCount="indefinite"/>
                    <animate attributeName="opacity" values="0.6;0;0.6" dur="1.6s" repeatCount="indefinite"/>
                  </circle>
                  <circle r="28" fill="rgba(0,245,196,0.18)" filter="url(#la-glow)" />
                </>
              )}
              <circle r="22" fill={isCurrent ? '#1F3B73' : '#121923'} stroke={isCurrent ? '#00F5C4' : '#1F3B73'} strokeWidth="2" />
              {isAccept && <circle r="17" fill="none" stroke={isCurrent ? '#7CFFB2' : '#1F3B73'} strokeWidth="1.5"/>}
      ...
      <div style={{
        ...
        background: 'rgba(18,25,35,0.84)', backdropFilter: 'blur(8px)',
        border: '1px solid #1F3B73', borderRadius: 10,
        fontFamily: 'JetBrains Mono, monospace', fontSize: 13,
      }}>
        <span style={{ color: '#7A8CA3', marginRight: 6 }}>input</span>
        {string.map((ch, i) => {
          ...
          return (
            <span key={i} style={{
              ...
              borderRadius: 5,
              background: isHead ? '#00F5C4' : consumed ? 'rgba(0,245,196,0.14)' : 'transparent',
              color: isHead ? '#061019' : consumed ? '#7CFFB2' : '#7A8CA3',
              border: `1px solid ${isHead ? '#7CFFB2' : consumed ? '#1F3B73' : 'transparent'}`,
        ...
        <span style={{
          ...
          background: accepted ? 'rgba(124,255,178,0.16)' : 'rgba(122,140,163,0.12)',
          color: accepted ? '#7CFFB2' : '#7A8CA3',
          border: `1px solid ${accepted ? 'rgba(124,255,178,0.4)' : '#1F3B73'}`,
        }}>
